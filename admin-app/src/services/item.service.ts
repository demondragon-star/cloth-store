...
class ItemService {
    ...
    async deleteItem(itemId: string): Promise<{ error: string | null }> {
        try {
            ...
            // 2. Manually delete related records first to ensure no orphans
            await Promise.all([
                supabase.from('item_variants').delete().eq('product_id', itemId),
                supabase.from('product_categories').delete().eq('product_id', itemId),
                supabase.from('cart_items').delete().eq('item_id', itemId),
                supabase.from('wishlist').delete().eq('item_id', itemId),
                supabase.from('product_waitlist').delete().eq('product_id', itemId),
                supabase.from('reviews').delete().eq('item_id', itemId),
                supabase.from(TABLES.ITEM_IMAGES).delete().eq('item_id', itemId)
            ]);
            ...
            // 4. Delete images from storage (optional cleanup)
